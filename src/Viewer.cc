...
void Viewer::Run() {
  ...
  while (1) {
    ...
    /*
        ...
        if ((menuShowTemplate)) {
          static_cast<DeformationMapDrawer
       *>(mpMapDrawer)->DrawTemplate(timestamp);
          if (menuShowTemplatehist)
            static_cast<DeformationMapDrawer
       *>(mpMapDrawer)->DrawTemplatehist();
        }*/
    if (mbSaveResults) {
    ...
    /* d_cam2.Activate(t_cam);
     if((menuShowTemplate)){
         static_cast<DeformationMapDrawer*>(mpMapDrawer)->DrawTemplateAtRest(timestamp);
     }
