...
namespace ORB_SLAM2
{
  ...
  void MapDrawer::UpdatePoints(Frame *pFrame, float s)
  {
    ...
    /*for (vector<MapPoint *>::const_iterator sit = vpRefMPs.begin(),
                                          send = vpRefMPs.end();
       sit != send; sit++) {
    if ((*sit)->isBad())
      continue;
 ...
 */
    ...
    for (size_t i = 0, iend = PointsSeen.size(); i < iend; i = i + 3)
    {
      ...
      if (PointsGT[i + 2] > 0.0)
      {
        double error = sqrt(pow(PointsGT[i] - PointsSeen[i], 2) +
                            pow(PointsGT[i + 1] - PointsSeen[i + 1], 2) +
                            pow(PointsGT[i + 2] - PointsSeen[i + 2], 2));
        double t = error / errorMax;
        double red[3] = {62.0 / 255.0, 37.0 / 255, 41.0 / 255};
        double green[3] = {62.0 / 255.0, 180.0 / 255.0, 81.0 / 255.0};
        //  std::cout << "error " << green[0] + (blue[0] - green[0]) * t << " "<<
        //  green[1] + (blue[1] - green[1]) * t << "" << green[2] + (blue[2] -
        //  green[2]) * t << std::endl;
  ...
  void MapDrawer::DrawError()
  {
    ...
    if (vpMPs.empty())
      return;
    float dcolor[3];
    double rgb1[3] = {140. / 255., 10. / 255., 0. / 255.};
    double rgb2[3] = {75. / 255., 215. / 255., 55. / 255.};
    float error_max = 0.06;
