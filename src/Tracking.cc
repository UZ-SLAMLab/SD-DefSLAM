...
namespace ORB_SLAM2
{
  ...
  Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer,
      ...
      : mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false),
        ...
        mpMap(pMap), mnLastRelocFrameId(0), mpLoopClosing(NULL),
        viewerOn(viewerOn)
  {
                                         ...
                                         const double &timestamp, cv::Mat _mask)
  {
    ...
    Track();
    timer.stop();
    std::cout << "Tracking time : " << timer.getElapsedTimeInMilliSec() << " ms"
              << std::endl;
    ...
    if (mState == eTrackingState::OK)
    {
      float scale =
          static_cast<defSLAM::GroundTruthFrame *>(mCurrentFrame)
              ->Estimate3DScale(
                  mpMap); // GroundTruthCalculator::CalculateScale(&mCurrentFrame);
                                           ...
                                           cv::Mat _mask)
  {
    ...
    timer.start();
    //  mCurrentFrame =
    //  GroundTruthFrame(mImGray,imGrayRight,timestamp,mpORBextractorLeft,
    // mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,imRectLeft,_mask);
    mCurrentFrame = new defSLAM::GroundTruthFrame(
        mImGray, timestamp, mpORBextractorLeft, mpORBVocabulary, mK, mDistCoef,
    ...
    if ((mState == eTrackingState::OK) && (false))
    {
      Timer timers;
      timers.start();
      float scale =
  ...
  void Tracking::Track()
  {
    ...
    //  timer.stop();
    // std::cout << "timer inside : " << timer.getElapsedTimeInMilliSec() << " ms"
    // << std::endl;
    ...
    // timer1.stop();
    // std::cout << "timer inside : " << timer1.getElapsedTimeInMilliSec() << "
    // ms" << std::endl;
  }
    ...
    if (mCurrentFrame->N > 100)
    {
      ...
      for (int i = 0; i < mCurrentFrame->N; i++)
      {
        ...
        if (z > 0)
        {
          ...
          MapPoint *pNewMP = new defSLAM::DeformationMapPoint(x3D, pKFini, mpMap);
  ...
  void Tracking::CreateInitialMapMonocular()
  {
    ...
    for (size_t i = 0; i < mvIniMatches.size(); i++)
    {
      ...
      MapPoint *pMP = new defSLAM::DeformationMapPoint(worldPos, pKFcur, mpMap);
  ...
  void Tracking::UpdateLastFrame()
  {
    ...
    for (size_t j = 0; j < vDepthIdx.size(); j++)
    {
      ...
      if (bCreateNew)
      {
        ...
        MapPoint *pNewMP = new defSLAM::DeformationMapPoint(x3D, mpMap, &mLastFrame, i);
  ...
  void Tracking::SearchLocalPoints()
  {
    ...
    for (uint i(0); i < mCurrentFrame->N; i++)
    {
      MapPoint *pMP = mCurrentFrame->mvpMapPoints[i];
      if (pMP)
      {
