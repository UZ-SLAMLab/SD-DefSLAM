...
DeformationFrameDrawer::DeformationFrameDrawer(Map *pMap) : FrameDrawer(pMap) {}

cv::Mat DeformationFrameDrawer::DrawFrame() {
...
void DeformationFrameDrawer::DrawTemplate(cv::Mat &im) {
  ...
  if ((mTcw.rows == 4) and (mTcw.cols == 4) and (mK.rows == 3) and
    ...
    for (std::set<Edge *>::iterator ite = Edges.begin(); ite != Edges.end();
         ite++) {
      std::pair<uint, uint> mo = (*ite)->get_pair_nodes();
      cv::Scalar S1, S2;
      uint a = M[mo.first];
      uint b = M[mo.second];
      Node *n1 = VecNodes[a];
...
void DeformationFrameDrawer::Update(Tracking *pTracker) {
  unique_lock<mutex> lock(mMutex);
  ...
  if (pTracker->mLastProcessedState == Tracking::NOT_INITIALIZED) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (pMP) {
        /* if (static_cast<DeformationMapPoint*>(pMP)->Deformable){
             vvbdeformable[i] =true;
         }*/
